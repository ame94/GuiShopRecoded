...
public class Tab {
    ...
    private List<ItemStack> sellItems = new ArrayList();
    private List<ItemStack> buyItems = new ArrayList();
    private List<ShopInventory> sellPages = new ArrayList();
    private List<ShopInventory> buyPages = new ArrayList();
    ...
    public static Tab createTab(String name, String configurationSection, String saleReason, String permission, ItemStack iconItem, double sale, boolean vip, List<ItemStack> buyItems, List<ItemStack> sellItems) {
        ...
        if(configurationSection == null) {
            ...
            return new Tab(name, configurationSection, saleReason, permission, iconItem, sale, vip, (List)buyItems, (List)sellItems);
        }
    }
    ...
    public static Tab loadTab(String configurationSection) {
        if(configurationSection == null) {
            ...
            FileConfiguration config = GuiShopRecoded.getTabs();
    ...
    public static Tab loadVipTab(int index) {
        if(index <= 3 && index >= 1) {
            FileConfiguration config = GuiShopRecoded.getMain();
            ...
            return createTab(name, configurationSection, saleReason, permission, iconItem, sale, true, buyItems, (List)null);
        } else {
            return null;
        }
    }
    ...
    public static List<ItemStack> loadItems(String configurationSection, boolean vip, OperationType oType) {
        ...
        FileConfiguration config = ConfigUtil.loadYaml(GuiShopRecoded.instance(), first + File.separator + configurationSection);
    ...
    private List<ShopInventory> generatePages(List<ItemStack> items, OperationType oType) {
        ...
        if(((List)items).isEmpty()) {
            ((List)items).add(new ItemStack(Material.AIR));
        }

        boolean havepages = ((List)items).size() > 42;

        for(int j = 0; (double)j < Math.ceil((double)((List)items).size() / 42.0D); ++j) {
            boolean lastpage = (double)(j + 1) >= Math.ceil((double)((List)items).size() / 42.0D);
            int strings = lastpage?((int)Math.ceil((double)(((List)items).size() - j * 42) / 7.0D) < 4 && havepages?4:(int)Math.ceil((double)(((List)items).size() - j * 42) / 7.0D)):6;
            ...
            if(this.vip) {
                ...
                inv.setItem(0, ItemUtil.load(GuiShopRecoded.getMain().getConfigurationSection(oType.name().toLowerCase() + "item")));
            }

            int slot;
            if(havepages) {
                ItemStack item = this.iconItem.clone();
                slot = (double)(j + 2) > Math.ceil((double)((List)items).size() / 42.0D)?(int)Math.ceil((double)((List)items).size() / 42.0D):j + 2;
                int prevp = j == 0?1:j;
            ...
            for(int i = j * 42; i < (j + 1) * 42; ++i) {
                ...
                ItemStack item = ((ItemStack)((List)items).get(i)).clone();
                if(item.hasItemMeta()) {
                    ItemMeta im = item.getItemMeta();
    ...
    public void closeTab() {
        List<ShopInventory> sinvs = new ArrayList();
        sinvs.addAll(this.getBuyPages());
        sinvs.addAll(this.getSellPages());
        Iterator var3 = sinvs.iterator();
    ...
    public void setSale(double sale, String saleReason) {
        this.sale = sale > 100.0D?1.0D:sale / 100.0D;
        this.saleReason = saleReason.replace("&", "§");
        FileConfiguration config = this.vip?GuiShopRecoded.getMain():GuiShopRecoded.getTabs();
        if(sale == 0.0D) {
        ...
        File file = new File(GuiShopRecoded.instance().getDataFolder(), path);
        GuiShopRecoded.instance().save(config, file);
        this.unregisterTab();
