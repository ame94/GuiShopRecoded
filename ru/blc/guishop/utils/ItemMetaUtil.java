...
public class ItemMetaUtil {
    ...
    public static Class<?> getNmsClass(String className) throws ClassNotFoundException {
        return Class.forName("net.minecraft.server." + GuiShopRecoded.instance().getServer().getClass().getPackage().getName().replace(".", ",").split(",")[3] + "." + className);
    }

    public static Class<?> getCraftbukkitClass(String className) throws ClassNotFoundException {
        return Class.forName("org.bukkit.craftbukkit." + GuiShopRecoded.instance().getServer().getClass().getPackage().getName().replace(".", ",").split(",")[3] + "." + className);
    }
        ...
        if(data == null) {
            ...
            if(item.getType() == Material.MONSTER_EGG) {
                try {
                    ...
                    GuiShopRecoded.instance().getServer().getLogger().log(Level.INFO, data);
                    return item.getItemMeta();
                }
