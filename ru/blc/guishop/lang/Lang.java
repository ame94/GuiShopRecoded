...
public class Lang {
    ...
    public static boolean installLanguage(String fileName) {
        FileConfiguration clang = ConfigUtil.loadYaml(GuiShopRecoded.instance(), "lang" + File.separator + fileName);
        if(isCorrect(clang)) {
            lang = clang;
    ...
    public static void printErrors(FileConfiguration clang, String fileName) {
        ...
        while(var3.hasNext()) {
            ...
            if(!clang.getKeys(false).contains(s)) {
                GuiShopRecoded.instance().getLogger().log(Level.INFO, "Missing phrase \"" + s + "\" at lang file \"" + fileName + "\"");
            }
        }
